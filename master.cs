...
public class master : Node2D
{
	...
	void init(){
		...
		if(!batchMode){
			...
			batchStack = dxionary.Count * batchSize;
		}
	}
	...
	public override void _Process(float delta)
	{
		...
		if(batchMode){
			if(batchStack > 0){
				if(!ongoing){
					// int batchSizet = batchSize;
					// int batchStackt = batchStack;
					// int selected = dropdown.Selected;
					destroy();
					dropdown.Selected = batchStack % dxionary.Count;
					// GD.Print("Stack : " + batchStack);
					// GD.Print("selected : " + batchStack % dxionary.Count);
					generateSimulation();
					Genetic();
					batchStack--;
				}
